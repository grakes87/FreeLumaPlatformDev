...
<objective>
Create the database foundation for the workshop system: 7 new Sequelize models with TypeScript interfaces, 8 migrations (7 tables + 1 User column), all associations registered in the model index, and install new npm dependencies (agora-rtc-react, agora-token, rrule).

Purpose: All workshop API and UI code depends on these models and tables existing first.
Output: 7 model files, 8 migration files, updated model index with associations, updated package.json.
</objective>
...
<task type="auto">
  <name>Task 2: Create 7 workshop models and 8 migrations</name>
  <files>
    src/lib/db/models/Workshop.ts
    src/lib/db/models/WorkshopSeries.ts
    src/lib/db/models/WorkshopCategory.ts
  ...
  <verify>Run `npx sequelize-cli db:migrate` — all 8 migrations should succeed. Check all 7 tables exist with correct columns via `npx sequelize-cli db:migrate:status`.</verify>
  <done>7 workshop tables created with proper schema, indexes, and constraints. User table has can_host column.</done>
</task>
...
2. `npx sequelize-cli db:migrate:status` — all 8 new migrations show as "up"
...
- 8 migration files create/modify tables correctly
