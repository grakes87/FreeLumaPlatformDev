...
<task type="auto">
  ...
  <action>
    ...
    onScreenShareToggle handler:
    - If starting: create `screenClient = AgoraRTC.createClient({ mode: 'live', codec: 'vp8' })`, set role to 'host', join channel with uid = user.id + 100000 and same token (re-fetch token for screen UID), create screen track via `AgoraRTC.createScreenVideoTrack()`, publish screen track to screenClient, emit socket event `workshop:screen-share-started` with { userId }, update local state `isScreenSharing = true`, render screen share overlay in video grid (screen share as main view, speaker video as PiP)
    - If stopping: unpublish screen track from screenClient, stop screen track (`screenTrack.close()`), leave channel on screenClient (`screenClient.leave()`), emit socket event `workshop:screen-share-stopped` with { userId }, update local state `isScreenSharing = false`
    - Screen share UID = user.id + 100000 (per research — separate from camera UID)
    - Handle browser's native "Stop sharing" button via screenTrack.on('track-ended') to trigger the stop flow automatically
