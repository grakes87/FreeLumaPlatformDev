...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/chat/MessageRequestBanner.tsx"
      to: "/api/chat/requests"
      via: "Accept/Decline calls POST /api/chat/requests"
      pattern: "fetch.*chat/requests"
---
...
**Access pattern:** Chat is accessed via a top bar icon (wired in Plan 03-13). The TopBar chat icon navigates to /chat. This plan creates the /chat page that the icon links to.
