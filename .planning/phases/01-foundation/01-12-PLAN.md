...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/app/(public)/bible/page.tsx"
      provides: "Faith-branded entry page for NFC bracelets and shared links"
      contains: "mode=bible"
    - path: "src/app/(public)/positivity/page.tsx"
      provides: "Positivity-branded entry page for NFC bracelets and shared links"
      contains: "mode=positivity"
  key_links:
    - from: "src/app/api/admin/activation-codes/route.ts"
      to: "src/lib/db/models/ActivationCode.ts"
      via: "bulk creates activation codes"
      pattern: "ActivationCode\\.bulkCreate"
    - from: "src/app/api/admin/daily-content/route.ts"
      to: "src/lib/db/models/DailyContent.ts"
      via: "creates/updates daily content records"
      pattern: "DailyContent\\.(create|update)"
---

<objective>
Build admin endpoints (activation codes + daily content management) and entry pages (/bible, /positivity).

Purpose: Admin endpoints are needed to populate daily content and generate activation codes. Entry pages support NFC bracelet and shareable link flows. Push notifications are deferred — the user decided push is for a future app version only, not the web platform.
Output: Admin CRUD for codes/content and branded entry pages.
</objective>
...
<task type="auto">
  <name>Task 1: Admin endpoints (activation codes and daily content management)</name>
  <files>
    src/app/api/admin/activation-codes/route.ts
    src/app/api/admin/daily-content/route.ts
  </files>
...
NOTE: A full admin panel UI is deferred. For Phase 1, these API endpoints are sufficient for managing content via curl or a REST client. The seeder (Plan 09) pre-populates sample content for development.
  </action>
  <verify>
```bash
# Login as admin
...
```
  ...
  <done>Admin endpoints for activation code generation and daily content scheduling operational. Non-admin users properly rejected.</done>
</task>

<task type="auto">
  <name>Task 2: /bible and /positivity branded entry pages</name>
  <files>
    src/app/(public)/bible/page.tsx
    src/app/(public)/positivity/page.tsx
  </files>
...
# http://localhost:3000/bible?activation_code=TESTCODE -- links to signup with params

# Build passes
npm run build
```
  </verify>
  <done>/bible and /positivity branded entry pages render with proper branding, NFC bracelet URL support (activation_code forwarding), and links to signup/login.</done>
</task>

</tasks>

<verification>
1. Admin can generate activation codes in bulk
2. Admin can schedule daily content
3. Non-admin users get 403 on admin endpoints
4. /bible entry page renders with faith branding
5. /positivity entry page renders with positivity branding
6. URL params (activation_code, mode) forward to signup
7. Build succeeds
</verification>
