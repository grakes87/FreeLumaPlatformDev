#!/usr/bin/env node
/**
 * Download positivity videos from freelumaquotes.com and upload to B2.
 * Updates daily_content.lumashort_video_url for positivity entries missing videos.
 *
 * Usage: node scripts/import-positivity-videos.mjs
 */
...
// Get all unique dates for positivity entries missing lumashort_video_url
...
console.log(`Found ${dates.length} dates with missing positivity videos\n`);
...
for (const date of dates) {
  ...
  // 2. Upload to B2
  const key = `daily-videos/positivity/${date}.mp4`;
  try {
  ...
  // 3. Update all positivity rows for this date
