...
const USERCOMMENTS_MAPPING = {
  ...
  columns: [
    ...
    { oldCol: 'is_liked', oldType: 'tinyint(1) DEFAULT 0', newTable: 'post_comment_reactions', newCol: 'reaction_type', newType: "ENUM('like','love','haha','wow','sad','pray')", transform: "Map: is_liked=1 -> reaction_type='love' (heart). Consistent with daily content import convention.", quality: '' },
...
const CHATS_MAPPING = {
  oldTable: 'chats',
  status: 'MAPPED',
  newTables: ['conversations', 'conversation_participants', 'messages', 'message_media', 'message_status'],
  notes: 'COMPLEX STRUCTURAL transformation. Flat chat table -> grouped conversations model. ~286 rows. Group by unique (min(sender,receiver), max(sender,receiver)) pairs. Each pair = 1 conversation + 2 participants. Each old row = 1 message. Media messages create message_media rows. Read status tracked via message_status table.',
  columns: [
    ...
    { oldCol: 'message', oldType: 'text NOT NULL', newTable: 'messages', newCol: 'content', newType: 'TEXT', transform: 'rename: message -> content', quality: 'Unicode escape sequences in some messages (e.g., U+2764 U+FE0F)' },
    { oldCol: 'message_type', oldType: "enum('TEXT','IMAGE','VIDEO','AUDIO') DEFAULT 'TEXT'", newTable: 'messages', newCol: 'type', newType: "ENUM('text','media','voice','shared_post','shared_video','system')", transform: "Map: TEXT->'text', IMAGE->'media', VIDEO->'media', AUDIO->'voice'. For IMAGE/VIDEO: also create message_media row.", quality: '' },
    { oldCol: 'media', oldType: 'varchar(255) DEFAULT NULL', newTable: 'message_media', newCol: 'media_url', newType: 'VARCHAR(500)', transform: "For non-NULL: create message_media row with local filename as media_url, media_type from message_type (IMAGE->'image', VIDEO->'video', AUDIO->'voice'), sort_order=0. POST-MIGRATION: separate script uploads files to B2 and updates media_url.", quality: 'NULL for TEXT messages, filename for media messages' },
    { oldCol: 'is_seen', oldType: 'tinyint(1) DEFAULT 0', newTable: 'message_status', newCol: 'status', newType: "ENUM('delivered','read')", transform: "is_seen=1: create message_status row (message_id, user_id=receiver_id, status='read', status_at=updatedAt). is_seen=0: create with status='delivered'.", quality: '' },
