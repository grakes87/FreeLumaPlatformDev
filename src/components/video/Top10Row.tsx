  ...
  return (
    <section className={cn('flex flex-col gap-2', className)}>
      ...
      <div
        ref={scrollRef}
        className={cn(
          'flex gap-6 overflow-x-auto px-4 pb-1',
          'scrollbar-none',
...
function Top10Card({ video, rank }: { video: VideoData; rank: number }) {
  return (
    <Link
      href={`/watch/${video.id}`}
      className="flex w-48 shrink-0 snap-start flex-col gap-1.5"
    >
      {/* Number + thumbnail+title column */}
      <div className="flex items-stretch gap-1">
        {/* Rank number — height constrained to thumbnail */}
        <span
          className={cn(
            'flex shrink-0 items-start overflow-hidden font-black leading-[1] tracking-tighter',
            'text-transparent',
            '[-webkit-text-stroke:3px_currentColor]',
