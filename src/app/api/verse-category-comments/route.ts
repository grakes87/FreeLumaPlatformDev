...
import { VerseCategoryComment, User } from '@/lib/db/models';
...
export const GET = withOptionalAuth(
  async (req: NextRequest, _context: OptionalAuthContext) => {
    try {
      ...
      const { count: total, rows: comments } = await VerseCategoryComment.findAndCountAll({
        where,
        include: [
          {
            model: User,
        ...
        attributes: {
          include: [
            [
              literal(
                `(SELECT COUNT(*) FROM verse_category_comments AS r WHERE r.parent_id = \`VerseCategoryComment\`.\`id\`)`
              ),
              'reply_count',
            ],
          ],
...
export const POST = withAuth(
  async (req: NextRequest, context: AuthContext) => {
    try {
      ...
      const { verse_category_content_id, parent_id, body: commentBody } = parsed.data;
      const user_id = context.user.id;

      // If parent_id is provided, verify it exists and belongs to the same content
      if (parent_id) {
        const parentComment = await VerseCategoryComment.findOne({
          where: { id: parent_id, verse_category_content_id },
        });
        if (!parentComment) {
      ...
      const comment = await VerseCategoryComment.create({
        user_id,
      ...
      const full = await VerseCategoryComment.findByPk(comment.id, {
        include: [
          {
            model: User,
            as: 'user',
