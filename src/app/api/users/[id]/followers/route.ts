 ...
 *
 * Privacy: private profiles only show followers to the owner.
...
export const GET = withAuth(async (req: NextRequest, context: AuthContext) => {
  try {
    ...
    // Private profiles: only the owner can see followers
    if (targetUser.profile_privacy === 'private' && targetId !== context.user.id) {
      return errorResponse('This user\'s followers are private', 403);
    }
    ...
    const followerIds = items.map((f) => {
      const u = (f as unknown as Record<string, unknown>).follower as { id: number } | null;
      return u?.id;
    }).filter(Boolean) as number[];

    const myFollows = followerIds.length > 0
    ...
    const enriched = items.map((f) => {
      ...
      const u = plain.follower as { id: number } | null;
      const uid = u?.id;
